...
export class FingerprintService {
  ...
  private processFile(filePath: string): string {
    try {
      ...
      const outputDir = dirname(join(this.publicDir, relative(this.assetsDir, filePath)));
    ...
    try {
      ...
      if (existsSync(sourceMapFile)) {
        ...
        content = content.replace(
          /\/\*# sourceMappingURL=.*? \*\//,
          `/*# sourceMappingURL=${fingerprintedMap} */`
        );
  ...
  public getAssetPath(assetPath: string): string {
    ...
    return this.manifest[normalizedPath] ? `/${this.manifest[normalizedPath]}` : assetPath;
  }
}
